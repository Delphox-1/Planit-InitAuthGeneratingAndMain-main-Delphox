  ...
  return (
    <>
      ...
      <div style={sidebarPanel}>
        ...
        <span style={sidebarItem} onClick={() => { onClose(); navigate("/mypage"); }}>
          마이페이지
        </span>
        <span style={sidebarItem} onClick={() => { onClose(); navigate("/study-stats"); }}>
          학습 통계
        </span>
        <span style={sidebarItemDisabled} title="준비중">챗봇 (준비중)</span>
        <div style={sidebarDivider} />
        <span style={sidebarItem} onClick={() => { onClose(); handleLogout(); }}>
          로그아웃
        </span>
      </div>
    </>
  ...
  if (error || !plan) {
    return (
      <div style={page}>
        ...
        <Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} navigate={navigate} handleLogout={handleLogout} />
        {error && (
