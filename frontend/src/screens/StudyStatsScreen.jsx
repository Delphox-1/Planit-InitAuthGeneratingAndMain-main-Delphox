...
const sidebarItemDisabled = { ...sidebarItem, color: "var(--ink-soft)", cursor: "not-allowed" };
const sidebarDivider = { height: 1, background: "var(--line)", margin: "8px 0" };
...
export default function StudyStatsScreen() {
  ...
  const pct = hasComparisonData ? Math.round((diff / a.comparisonAvgMinutes) * 100) : 0;
        ...
        <div className="dash-analysis-row">
          ...
          <div className="analysis-card">
            ...
            <div className="callout">
              <span className="q">"</span>
              <span>
                가장 낮은 지표는 <b>{radarLabels[minIdx]}({radarValues[minIdx]}%)</b>,
                가장 높은 지표는 <b>{radarLabels[maxIdx]}({radarValues[maxIdx]}%)</b>이에요!"
              </span>
            </div>
          </div>
        </div>
