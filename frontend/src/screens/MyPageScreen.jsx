...
const Icon = {
  ...
  mail: (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="m2 7 10 6 10-6"></path>
    </svg>
  ),
  photo: (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="3" y="3" width="18" height="18" rx="4"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><path d="m21 15-5-5L5 21"></path>
    </svg>
  ),
  lock: (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  trash: (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M10 11v6"></path><path d="M14 11v6"></path><path d="M4 7h16"></path>
      <path d="M6 7V4a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v3"></path><path d="M6 7l1 13a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2l1-13"></path>
    </svg>
  ),
};
...
export default function MyPageScreen() {
  ...
  const handleResetPassword = async () => {
    if (!email) return setMsg({ type: "err", text: "이메일 정보를 불러오지 못했어요." });
    try {
      await sendPasswordResetEmail(auth, email);
      setMsg({ type: "ok", text: `${email}로 비밀번호 재설정 메일을 보냈어요.` });
    } catch (e) {
  ...
  const handleWithdraw = async () => {
    if (!window.confirm("정말 탈퇴하시겠습니까?\n계정과 학습 데이터가 모두 삭제되며 되돌릴 수 없습니다.")) return;
    try {
      await fetch(`${AUTH_API_BASE}/api/auth/withdraw`, { method: "POST", credentials: "include" });
      localStorage.removeItem("userId");
      window.location.href = "/";
    } catch (e) {
      setMsg({ type: "err", text: "탈퇴 처리 중 오류가 발생했어요: " + e.message });
    }
  };

  if (!memberId) {
    return <div className="mypage-root"><p>로그인이 필요해요.</p></div>;
  }
  if (loading) {
              ...
              <div className="mypage-card-body">
                <div className="mypage-row">
                  <div className="mypage-row-label">
                    ...
                    <div className="mypage-row-text"><div className="t">이름</div><div className="d">{name}</div></div>
                  </div>
                  <button className="mypage-btn mypage-btn-ghost" onClick={handleEditName}>수정</button>
                </div>
                <div className="mypage-row">
                  <div className="mypage-row-label">
                    <div className="mypage-row-icon">{Icon.mail}</div>
                    <div className="mypage-row-text"><div className="t">이메일</div><div className="d">{email}</div></div>
                  </div>
                </div>
                <div className="mypage-row">
                  <div className="mypage-row-label">
                    <div className="mypage-row-icon">{Icon.photo}</div>
                    <div className="mypage-row-text"><div className="t">프로필 사진</div><div className="d">기본 이미지 사용 중</div></div>
                  </div>
                  <button className="mypage-btn mypage-btn-ghost" onClick={handlePhotoChange}>변경</button>
                </div>
              </div>
            </section>
              ...
              <div className="mypage-card-body">
                <div className="mypage-row">
                  <div className="mypage-row-label">
                    ...
                    <div className="mypage-row-text"><div className="t">비밀번호 변경</div><div className="d">이메일로 재설정 링크를 보내드려요</div></div>
                  </div>
                  <button className="mypage-btn mypage-btn-primary" onClick={handleResetPassword}>변경</button>
                </div>
                <div className="mypage-row mypage-danger-row">
                  <div className="mypage-row-label">
                    <div className="mypage-row-icon">{Icon.trash}</div>
                    <div className="mypage-row-text"><div className="t">회원 탈퇴</div><div className="d">탈퇴 시 모든 학습 데이터가 삭제되고 복구할 수 없어요</div></div>
                  </div>
                  <button className="mypage-btn mypage-btn-danger" onClick={handleWithdraw}>탈퇴하기</button>
                </div>
              </div>
            </section>
          </div>
